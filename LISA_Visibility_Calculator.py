...
Vis_DNSB_Val = pd.read_csv(r'Data\Visible_DNSB_Values(300_2_upper200).csv')
Vis_DNSB_Dat = pd.read_csv(r'Data\Visible_DNSB_Data(300_2_upper200).csv')
#Fin are final positions R and z
Vis_DNSB_Fin = pd.read_csv(r'Data\Final_Position(300_2_upper200).csv')
#length of the list
...
ax2.plot(F_background, F3PSD(F_background), 'r',
         linestyle='solid', linewidth=1, label='Background')
...
ax2.scatter(1, 1, color='k', s=65, marker='^', label='DNSB population')
#DNSB
...
ax3.plot(F_background, F3(F_background), 'r',
         linestyle='solid', linewidth=1, label='Background')
...
ax3.scatter(1, 1, color='k', s=65, marker='^', label='DNSB population')
#DNSB
...
ax9.set(xlabel=u'$\mathregular{M_{total}/M_\odot}$',
        ylabel=u'$\mathregular{Strain\:\:\:\:Sensitivity\:\:\:\:[\sqrt{Hz}^{-1}]}$')
ax9.set_xlim(2.5, 5.05)
ax9.set_ylim(1e-22, 4e-17)
ax9.set_yscale('log')
plt.gcf().set_size_inches(15, 15)
...
ax10.set(xlabel=u'$\mathregular{r/kpc}$',
         ylabel=u'$\mathregular{Strain\:\:\:\:Sensitivity\:\:\:\:[\sqrt{Hz}^{-1}]}$')
ax10.set_xlim(5e-2, 1e4)
ax10.set_ylim(1e-22, 4e-17)
ax10.loglog()
ax10.invert_xaxis()
plt.gcf().set_size_inches(15, 15)
...
for i in range(0, List_len):
    if(DNSB_SNR_noBackground[i] > 5):
        RB_f_gw_noBack.append(DNSB_f_gw[i])
        RB_h_c_noBack.append(DNSB_h_c[i])
    if(DNSB_SNR[i] > 5):
        RB_f_gw.append(DNSB_f_gw[i])
        RB_h_c.append(DNSB_h_c[i])
...
ax4.scatter(RB_f_gw_noBack, RB_h_c_noBack, color='k',
            lw = 0.6, s = 40, alpha = 0.75, marker='^')
#DWDB
...
#%%
"""
PLOTTING THE ELLIPTICAL PROJECTION (FULL DNSB Population)
"""
#CMAP
pos_color_log = []
pos_color = []
for i in range(0, List_len):
    pos_color_log.append(math.log(r[i], 10))
    pos_color.append(math.log(r[i], 10))
...
im = ax.scatter(l, b, cmap=cmr.get_sub_cmap('gnuplot2', 0.1, 0.75),
                c=pos_color_log, vmin=-0.5, vmax=2.5, lw = 0.5, s = 4.5, alpha= 0.45)
...
cb = plt.colorbar(im, orientation='vertical', fraction=0.02345, pad=0)
...
#fig.savefig('Elliptical_projection_DNSB.png', dpi=300, bbox_inches='tight')
plt.show()
#%%
"""
PLOTTING THE ELLIPTICAL PROJECTION (RBs)
"""
#CMAP
RB_l = []
RB_b = []
...
ax.grid(True, which='both', alpha = 0.7)
ax.set_axisbelow(True)
ax.scatter(l, b, c='gray', lw = 0.5, s = 4.5, alpha= 0.45)
#Plotting the resolvable DNSBs
im = ax.scatter(RB_l, RB_b, cmap=cmr.get_sub_cmap('gnuplot2', 0.2, 0.8),
                c=pos_color_log, vmin = -0.5 ,vmax = 1.5, s=90, marker='^')
...
cb.set_label(r'log(r)/kpc')
ax.legend(loc=(0.825, 0.02))
#fig.savefig('Elliptical_projection_RB.png', dpi=300, bbox_inches='tight')
plt.show()
#%%
"""
PLOTTING THE TOP-DOWN VIEW (Full DNSB Population)
"""
#CMAP
RB_X = []
RB_Z = []
...
im11 = ax11.scatter(X_sol, Z,cmap=cmr.get_sub_cmap('cmr.ember_r', 0.15, 0.8, N=10),
                    c=pos_color, vmin = 0, vmax = 50, lw = 1, s = 2, alpha= 0.2)
...
im12 = ax12.scatter(X_sol, Z, cmap=cmr.get_sub_cmap('cmr.ember_r', 0.15, 0.8, N=10),
                    c=pos_color, vmin = 0, vmax = 50, lw = 1, s = 2, alpha= 0.2)
...
cax12 = ax12_divider.append_axes("top", size="2.45%", pad=-2.76)
...
ax12.minorticks_on()
ax12.set_xlim(-10.1-8.1, 15.6-8.1)
ax12.set_ylim(-15, 15)
ax12.set_aspect(0.254)
cax12.xaxis.set_ticks_position("top")
ax12.set(ylabel=u'$\mathregular{z/kpc}$')
ax12.set_title(u'$\mathregular{r/kpc}$', y=1.25)
ax12_divider.append_axes("top", size="0%", pad=2.76)

ax9.scatter(X_sol, Y, cmap=cmr.get_sub_cmap('cmr.ember_r', 0.15, 0.8, N=10),
            c=pos_color, vmin = 0, vmax = 50, lw = 1, s = 2, alpha= 1)
...
ax10.scatter(X_sol, Y, cmap=cmr.get_sub_cmap('cmr.ember_r', 0.15, 0.8, N=10),
            c=pos_color, vmin = 0, vmax = 50, lw = 1, s = 2, alpha= 1)
...
ax10.set_xlim(-10.1-8.1, 15.6-8.1)
ax10.set_ylim(-15, 15)
plt.gcf().set_size_inches(15, 15)
...
#%%
"""
PLOTTING THE TOP-DOWN VIEW (RBs)
"""
#CMAP
RB_X = []
RB_Z = []
...
ax11.scatter(X_sol, Z, c='gray',
                  lw = 1, s = 2, alpha= 0.2)
im11 = ax11.scatter(RB_X, RB_Z, cmap=cmr.get_sub_cmap('cmr.ember_r', 0, 0.9, N=8),
                    c=pos_color, vmin = 0, vmax = 20, s=80, marker='^')
ax11.scatter(-8.33, 0, s=120, marker='x', color='k', label = 'Galactic center')
ax11.scatter(0, 0, s=200, marker='*', color='k', label = 'Solar System')
ax11.tick_params(direction='in', length=10, width=1.5, grid_alpha=0.55,
                which = 'major', top=True, right=True, pad=7)
...
ax12.scatter(X_sol, Z, c='gray',
                  lw = 1, s = 2, alpha= 0.2)
im12 = ax12.scatter(RB_X, RB_Z, cmap=cmr.get_sub_cmap('cmr.ember_r', 0, 0.9, N=8),
                    c=pos_color, vmin = 0, vmax = 20, s=80, marker='^')
ax12.scatter(-8.33, 0, s=120, marker='x', color='k', label = 'Galactic center')
ax12.scatter(0, 0, s=200, marker='*', color='k', label = 'Solar System')
ax12.tick_params(direction='in', length=10, width=1.5, grid_alpha=0.55,
...
ax9.scatter(X_sol, Y, c='gray',
                  lw = 1, s = 2, alpha= 0.5)

ax9.scatter(RB_X, RB_Y, cmap=cmr.get_sub_cmap('cmr.ember_r', 0, 0.9, N=8),
            c=pos_color, vmin = 0, vmax = 20, s=80, marker='^')
ax9.scatter(-8.33, 0, s=120, marker='x', color='k', label = 'Galactic center')
ax9.scatter(0, 0, s=200, marker='*', color='k', label = 'Solar System')
ax9.scatter(220, 220, s=120, marker='^', color='k', label = 'Resolvable DNSBs')
...
ax10.scatter(X_sol, Y, c='gray',
                  lw = 1, s = 2, alpha= 0.5)

ax10.scatter(RB_X, RB_Y, cmap=cmr.get_sub_cmap('cmr.ember_r', 0, 0.9, N=8),
             c=pos_color, vmin = 0, vmax = 20, s=80, marker='^')
ax10.scatter(-8.33, 0, s=120, marker='x', color='k', label = 'Galactic center')
ax10.scatter(0, 0, s=200, marker='*', color='k', label = 'Solar System')
ax10.scatter(220, 220, s=120, marker='^', color='k', label = 'Resolvable DNSBs')
