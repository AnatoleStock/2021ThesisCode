...
#From McMillan (2018)
sigma_0 = 887
#The normalizing factor is taken after evaluating the function C(R) for some
#arbitrarely large R.
...
def Stellar_distribution(k):
    ...
    for h in range(0, k):
        ...
        if (x[h] < 0):
            print(x[h], y[h], h)
    return (x, y)#the usefull value here is x, which is the actual distance
#%%
"""Testing the method"""
(x, y) = Stellar_distribution(300)#with population of 300 in this case
x[:] = [b/1000 for b in x]
...
ax.set_ylim(0,900)
ax.set_box_aspect(1)
ax.set(ylabel=u'$\mathregular{\Sigma (R)}$')
...
plt.gcf().set_size_inches(15, 15)
#fig.savefig('GSD_plot.png', dpi=300, bbox_inches='tight')
plt.show()
